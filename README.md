...
> RAGAS internally использует OpenAI API для вычисления метрик оценки.  
> Даже если RAG pipeline работает через ProxiAPI, метрики RAGAS требуют `OPENAI_API_KEY`.  
> Это ограничение библиотеки RAGAS v0.4+, которая использует OpenAI модели для оценки.
...
| **ProxiAPI** | `PROXI_API_URL`, `PROXI_API_KEY` (опционально) |
