"""
Основной RAG pipeline для ProxiAPI режима.
...
class RAGPipeline:
    """Основной pipeline для RAG системы в ProxiAPI режиме."""
    ...
    def __init__(self, 
        ...
        Args:
            ...
            model: модель через ProxiAPI для генерации ответов
        """
        # Проверка API URL
        ...
        self.model = model
        self.proxy_client = ProxyAPIClient(default_model=model)
        ...
        print("RAG Pipeline инициализирован (ProxiAPI mode)")
...
Ответ:"""
    ...
    def _generate_answer(self, prompt: str) -> str:
        """
        Генерация ответа через ProxiAPI.
        ...
        answer = self.proxy_client.chat_completion(
            messages=messages,
            model=self.model,
    ...
    def query(self, user_query: str, use_cache: bool = True) -> Dict[str, Any]:
        """
        Основной метод для обработки запроса пользователя через ProxiAPI.
        ...
        print("[*] Поиск релевантных документов через ProxiAPI...")
        ...
        # Шаг 4: Генерация ответа через ProxiAPI
        print(f"[*] Генерация ответа через ProxiAPI ({self.model})...")
        answer = self._generate_answer(prompt)
        print("[+] Ответ получен от ProxiAPI")
...
if __name__ == "__main__":
    # Тестирование RAG pipeline в ProxiAPI режиме
    import sys
